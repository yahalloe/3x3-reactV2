...
export function EditorialComments({ busy, runAction }: { busy: boolean; runAction: (saving: string, success: string, action: () => Promise<void>) => Promise<void> }) {
  ...
  const choose = (entry: Anime) => { setSelectedId(entry.id); setPreview(false); };
  const save = () => {
    if (!selected) return;
  ...
  return <div className="editor-columns"><AnimeLibrary anime={anime} selectedId={commentId} onSelect={choose} />
    <section className="admin-panel min-w-0"><SectionTitle title="Your comments">Your personal take on each show, displayed on its public anime page.</SectionTitle>
      {selected ? <>
        ...
        {saved && <article className="mb-6 flex gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-cyan-300/15 text-sm font-bold text-cyan-300" aria-hidden="true">Y</span><div className="min-w-0 flex-1 border-l border-white/10 pl-4"><p className="text-xs"><strong>Yahallo</strong><span className="ml-2 rounded bg-cyan-300/10 px-1.5 py-0.5 text-cyan-300">Author</span><span className="ml-2 text-zinc-500">Published comment</span></p><p className="mt-3 whitespace-pre-wrap break-words text-sm leading-7 text-zinc-300">{saved}</p><button type="button" onClick={() => { setPreview(false); document.getElementById("comment-draft")?.focus(); }} className="editor-text-button mt-3">✎ Edit comment</button></div></article>}
        <form onSubmit={(event) => { event.preventDefault(); save(); }}>
          <div className="overflow-hidden rounded-2xl border border-white/15 bg-zinc-900/50">
            <div className="flex items-center justify-between border-b border-white/10 px-4 py-3"><label htmlFor="comment-draft" className="text-sm font-semibold">{saved ? "Edit your comment" : "Add your take"}</label><div className="flex gap-1">{[false, true].map((isPreview) => <button type="button" key={String(isPreview)} aria-pressed={preview === isPreview} onClick={() => setPreview(isPreview)} className={`rounded-full px-3 py-1 text-xs ${preview === isPreview ? "bg-white/10 text-white" : "text-zinc-500"}`}>{isPreview ? "Preview" : "Write"}</button>)}</div></div>
            {preview ? <p className="min-h-48 whitespace-pre-wrap break-words p-4 text-sm leading-7 text-zinc-300">{draft || "Your comment preview will appear here."}</p>
              : <textarea id="comment-draft" aria-label="Your comment" className="min-h-48 w-full resize-y bg-transparent p-4 text-sm leading-7 outline-none focus:ring-2 focus:ring-inset focus:ring-cyan-300/30" placeholder="What stayed with you about this show?" value={draft} onChange={(event) => setDrafts((current) => ({ ...current, [commentId]: event.target.value }))} onKeyDown={(event) => { if ((event.ctrlKey || event.metaKey) && event.key === "Enter") { event.preventDefault(); if (!busy) save(); } }} />}
